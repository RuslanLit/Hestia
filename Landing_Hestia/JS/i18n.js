(function () {
  ...
  const copy = {
    en: {
      ...
      server: {
        ...
        intro: "Run a Hestia server for your own users. The server relays encrypted traffic and call signaling; plaintext messages and files should not be stored on the server.",
        ...
        sections: [
          ...
          ["What you need before starting", ["VPS or server with shell access.", "Domain or subdomain.", "Node.js and npm.", "Git.", "Firewall access.", "Optional Nginx or Caddy reverse proxy.", "Optional Firebase config for Android push.", "Optional TURN server for call relay fallback."]],
          ["Get the server code", ["Replace the repository placeholder with the real Hestia server repository URL when it is published."], "git clone <HESTIA_SERVER_REPOSITORY_URL> hestia-server\ncd hestia-server\nnpm install"],
          ...
          ["Start the server", ["The server should expose /config, accept WebSocket connections, and have writable persistence storage."], "npm start\ncurl http://127.0.0.1:3000/config"],
          ["Reverse proxy, HTTPS, and WSS", ["For production, put Hestia behind a TLS reverse proxy. WebSocket upgrade support is required for realtime messaging and signaling."], "server {\n    listen 443 ssl http2;\n    server_name chat.example.com;\n    ssl_certificate /path/to/fullchain.pem;\n    ssl_certificate_key /path/to/privkey.pem;\n    location / {\n        proxy_pass http://127.0.0.1:3000;\n        proxy_http_version 1.1;\n        proxy_set_header Host $host;\n        proxy_set_header Upgrade $http_upgrade;\n        proxy_set_header Connection \"upgrade\";\n    }\n}"],
          ["Optional TURN for calls", ["Direct WebRTC works best. Restrictive networks may require TURN relay fallback through TURN_SERVERS."], "TURN_SERVERS=[{\"urls\":\"turn:turn.example.com:3478\",\"username\":\"<TURN_USERNAME>\",\"credential\":\"<TURN_PASSWORD>\"}]"],
          ["Connect the client", ["Open Hestia, enter the server URL if required, register or log in, then test messaging and calls."]],
          ["Verification checklist", ["/config is reachable over HTTPS.", "WebSocket connects over WSS.", "Registration or login works.", "Two users can exchange messages.", "Offline delivery, file transfer, and call signaling work."]],
  ...
  const privacySections = {
    ru: [
      ...
      ["Запустить сервер", ["Сервер должен отдавать /config, принимать WebSocket и иметь доступ на запись к persistence storage."], serverCodes[4]],
      ["Reverse proxy, HTTPS и WSS", ["Для production разместите Hestia за TLS reverse proxy. WebSocket upgrade обязателен для realtime messaging и signaling."], serverCodes[5]],
      ...
      ["Подключить клиент", ["Откройте Hestia, введите URL сервера при необходимости, зарегистрируйтесь или войдите, затем проверьте сообщения и звонки."]],
      ["Проверочный список", ["/config доступен по HTTPS.", "WebSocket подключается по WSS.", "Регистрация или вход работают.", "Два пользователя обмениваются сообщениями.", "Offline delivery, передача файлов и call signaling работают."]],
    ...
    uk: [
      ...
      ["Запустити сервер", ["Сервер має віддавати /config, приймати WebSocket і мати запис у persistence storage."], serverCodes[4]],
      ["Reverse proxy, HTTPS і WSS", ["Для production розмістіть Hestia за TLS reverse proxy. WebSocket upgrade потрібен для realtime messaging і signaling."], serverCodes[5]],
      ...
      ["Підключити клієнт", ["Відкрийте Hestia, введіть URL сервера за потреби, зареєструйтеся або увійдіть, потім перевірте повідомлення й дзвінки."]],
      ["Список перевірки", ["/config доступний через HTTPS.", "WebSocket підключається через WSS.", "Реєстрація або вхід працюють.", "Два користувачі обмінюються повідомленнями.", "Offline delivery, файли та call signaling працюють."]],
    ...
    pl: [
      ...
      ["Uruchom serwer", ["Serwer powinien udostępniać /config, przyjmować WebSocket i mieć zapis do persistence storage."], serverCodes[4]],
      ["Reverse proxy, HTTPS i WSS", ["W produkcji umieść Hestia za TLS reverse proxy. WebSocket upgrade jest wymagany dla realtime messaging i signaling."], serverCodes[5]],
      ...
      ["Połącz klienta", ["Otwórz Hestia, wpisz URL serwera, jeśli trzeba, zarejestruj się lub zaloguj, potem przetestuj wiadomości i rozmowy."]],
      ["Lista weryfikacji", ["/config jest dostępny przez HTTPS.", "WebSocket łączy się przez WSS.", "Rejestracja lub logowanie działa.", "Dwóch użytkowników wymienia wiadomości.", "Offline delivery, pliki i call signaling działają."]],
    ...
    es: [
      ...
      ["Iniciar el servidor", ["El servidor debe exponer /config, aceptar WebSocket y poder escribir en persistence storage."], serverCodes[4]],
      ["Reverse proxy, HTTPS y WSS", ["En producción, coloca Hestia detrás de un TLS reverse proxy. WebSocket upgrade es necesario para realtime messaging y signaling."], serverCodes[5]],
      ...
      ["Conectar el cliente", ["Abre Hestia, introduce el URL del servidor si hace falta, regístrate o inicia sesión, y prueba mensajes y llamadas."]],
      ["Lista de verificación", ["/config es accesible por HTTPS.", "WebSocket conecta por WSS.", "Registro o login funciona.", "Dos usuarios intercambian mensajes.", "Offline delivery, archivos y call signaling funcionan."]],
    ...
    cs: [
      ...
      ["Spustit server", ["Server má poskytovat /config, přijímat WebSocket a mít zápis do persistence storage."], serverCodes[4]],
      ["Reverse proxy, HTTPS a WSS", ["V produkci dejte Hestia za TLS reverse proxy. WebSocket upgrade je nutný pro realtime messaging a signaling."], serverCodes[5]],
      ...
      ["Připojit klienta", ["Otevřete Hestia, zadejte URL serveru, pokud je potřeba, registrujte se nebo přihlaste a otestujte zprávy a hovory."]],
      ["Kontrolní seznam", ["/config je dostupný přes HTTPS.", "WebSocket se připojí přes WSS.", "Registrace nebo přihlášení funguje.", "Dva uživatelé si vymění zprávy.", "Offline delivery, soubory a call signaling fungují."]],
    ...
    de: [
      ...
      ["Server starten", ["Der Server sollte /config bereitstellen, WebSocket akzeptieren und Schreibzugriff auf persistence storage haben."], serverCodes[4]],
      ["Reverse Proxy, HTTPS und WSS", ["In Produktion Hestia hinter einen TLS reverse proxy stellen. WebSocket upgrade ist für realtime messaging und signaling nötig."], serverCodes[5]],
      ...
      ["Client verbinden", ["Hestia öffnen, bei Bedarf Server-URL eingeben, registrieren oder anmelden, dann Nachrichten und Anrufe testen."]],
      ["Prüfliste", ["/config ist über HTTPS erreichbar.", "WebSocket verbindet über WSS.", "Registrierung oder Login funktioniert.", "Zwei Nutzer können Nachrichten austauschen.", "Offline delivery, Dateiübertragung und call signaling funktionieren."]],
  ...
  function renderDownloads() {
    ...
    document.querySelector("main").innerHTML = `
      ...
      <section class="section release-section" aria-labelledby="release-title">
        ...
        <div class="release-toolbar"><a class="release-link" href="${release.releaseNotesUrl || "#"}">${escapeHtml(d.notes)}</a><a class="release-link" href="${release.checksumUrl || "#"}">${escapeHtml(d.checksums)}</a><a class="release-link" href="${linkFor("serverGuide")}">${escapeHtml(d.serverGuide)}</a></div>
