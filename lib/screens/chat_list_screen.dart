...
class _ChatListScreenState extends State<ChatListScreen>
  ...
  Future<void> _showDevices() async {
    ...
    await showDialog<void>(
      context: context,
      builder: (_) => const _DevicesDialog(),
    );
  }
            ...
            : ListView.separated(
                ...
                itemBuilder: (_, index) {
                  ...
                  final lastActive = session.lastActiveAt == null
                      ...
                      : l10n.lastActive(
                          session.lastActiveAt!.toLocal().toString(),
                        );
                  return ListTile(
                    leading: Icon(
                      session.current
                    ...
                    title: Text(
                      session.current
                          ? l10n.currentDevice(session.deviceName)
                          : session.deviceName,
                    ),
                    subtitle: Text('${session.platform} · $lastActive'),
  ...
  Widget build(BuildContext context) {
    ...
    return ListView.separated(
      ...
      itemBuilder: (_, index) {
        ...
        final subtitle = last?.attachment != null
            ? context.l10n.attachmentNamed(last!.attachment!.name)
            : last?.text ?? '';
