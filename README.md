...
Android 8.0/API 26 and newer are supported. Android can stop or suspend
background WebSocket work when the app is in the background, the screen is off,
battery optimization is active, or the app has been force-stopped. Hestia
therefore prefers FCM data push when Google Play Services are available and
falls back to an Android foreground-service WebSocket when FCM is unavailable.
Android 13+ asks for notification permission; Android 8-12 continue with normal
